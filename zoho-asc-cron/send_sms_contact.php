...
if($objZoho->checkTokens())
{
	...
	{
		$arrParams=[];
		$arrParams['cvid']="3108913000008200011";
		$arrParams['page']=$i;
		$arrParams['per_page']=200; 
		$respSearchContacts=$objZoho->getRecords("All_Contacts",$arrParams);	
		if(isset($respSearchContacts['data']))
		{
			if(count($respSearchContacts['data'])>0)
			{	
				$arrBatchSms=[];
				foreach ($respSearchContacts['data'] as $key => $arrParent) 
				{
					$arrSms=[];	
					$arrSms['to_number']=str_replace(" ","",$arrParent['Primary_Phone']);
					$arrSms['from_number']="61488824543";
					$arrSms['message']=$arrParent['SMS_Text'];
					$arrSms['reference']="";
					$arrSms['contact_type']="contact";
					$arrSms['contact_id']=$arrParent['id'];
					$arrSms['sms_opt_out']=$arrParent['Sms_opt_out'];
					$arrSms['schedule_time']=$arrParent['Schedule_SMS'];
	...
	$rsltSmsLog=$objSMSLog->getSMSFromQueue('contact');
...
if($objZoho->checkTokens())
{
	...
	$arrAllSmsRecords=[];
	$arrAllParents=[];
	$rsltProcessesSms=$objSMSLog->getSMSFromProcessed('contact');
	while($rowProcesses=$rsltProcessesSms->fetch_assoc())
	{
		...
		$arrSmsRecord['Contact_Name']=$rowProcesses['contact_id'];
		...
		if($recordCount>=95)
		{
			...
			$resp=$objZoho->bulkUpdateRecords("All_Contacts",$arrAllParents);
			$arrAllParents=[];
		}
	}
