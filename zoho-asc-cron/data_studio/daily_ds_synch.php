...
try
{
	...
	if($objZoho->checkTokens())
	{
			...
			if(isset($resp['data']))
			{
				if(count($resp['data'])>0)
				{
					foreach ($resp['data'] as $key => $arrZohoBooking) 
					{
						...
						$arrDBBooking['parent_type']=$arrZohoBooking['Customer_Type'];
						$arrDBBooking['family_type']=$arrZohoBooking['Family_Type'];
		...
		// ************ Calculate season and year for all participants ***********
		echo "<br/><br/>Calculate dat and week for all participants.";
		include "calculate_day_week.php";
	}
	else
	{
