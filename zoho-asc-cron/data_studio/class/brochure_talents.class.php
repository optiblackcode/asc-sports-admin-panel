...
class BROCHURE_TALENTS extends DB
{
	private $table="brochure_talents";
	// *************** Get talent by rec_id of database *************************
	public function getTalentById($rec_id)
	{
	...
	public function searchTalents($arrParams=[])
	{
		...
		{
			$arrWhereClause[]="talent_name LIKE '%{$arrParams['name']}%'";
		}
		if(isset($arrParams['sports']) && !empty($arrParams['sports']))
		{
			$arrWhereClause[]="talent_sports LIKE '%{$arrParams['sports']}%'";
		}
		...
		// Order by clause
		$orderBy=" ORDER BY talent_name ASC";
	...
	public function updateTalent($id,$arrTalent,$arrExtraParameters=null)
	{
		...
		$rsltUpd=$this->common_update($this->table,$arrTalent,"WHERE rec_id='{$id}'",$arrExtraParameters);
		if($rsltUpd)
		{
			return $rsltUpd;
		}
	...
	public function GetAllData($suburb,$sports,$state)
	{
		
		$qrySel="SELECT Camps_Abilities,ASC_Overview FROM is_partner_sample_data WHERE Suburb LIKE '%$suburb%' AND Sports LIKE '%$sports%' AND State LIKE '%$state%' ";
		$rsltsampleday=$this->conn->query($qrySel);
		return $rsltsampleday;
	}
}
