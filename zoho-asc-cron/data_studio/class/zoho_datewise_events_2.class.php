...
class ZOHO_DATEWISE_EVENTS_2 extends DB
{
	private $table="zoho_datewise_events_2";
	...
	// *************** Get events by date*************************
	public function getEventsByDateAndCategory($event_date,$event_category)
	{
		...
		$qrySel="SELECT e.*,p1.name created_by_name,p2.name modified_by_name
					...
					AND e.event_category='{$event_category}'
					AND e.is_deleted!='1'";
		$rsltSel=$this->conn->query($qrySel);
		if($rsltSel)
		{
	...
	// *************** Get all events by season_year **********
	public function getAllDatesBySeasonYear($season="",$year="")
	{
		$qrySel="SELECT e.*,p1.name created_by_name,p2.name modified_by_name
					FROM $this->table e
					LEFT JOIN  portal_user p1
	...
	public function deleteRecord($id)
	{
		...
		$arrEvent=[];
		$arrEvent['is_deleted']=1;
		$arrEvent['deleted_by']=$userId;
		$arrEvent['deleted_at']=date("Y-m-d H:i:s");
